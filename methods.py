...
a lista de substituilções de um cardápio.
...
def getResults(context, similarityMatrix = None, refCode = None, menu = None, foods = None, grouping = None):
    ...
    if context == "food":
        # Obtém a linha da matriz referente ao alimento
        foodRow = next((row for row in similarityMatrix if row[0]["food_i"]["code"] == refCode), None)

        result = {
            "referenceFood": foodRow[0]["food_i"],
            "replacements": []
        }

        for col in foodRow:
            if (col["food_i"]["code"] != col["food_j"]["code"]):
                result["replacements"].append({"food": col["food_j"], "score": col["score"], "semaphore": col["semaphore"]})
